...
/**
 * `addr_hint`(시/군/구) 단위 확대 지도용 경계 윤곽선.
 * `public/data/city_outlines/*.json`에서 라운드마다 필요한 것만 지연 로드한다
 * (전국 230개 시/군/구를 한 번에 받지 않음). `rings`의 각 점은 `[lng, lat]`
 * 순서 - GeoJSON/TopoJSON 관례를 그대로 따른다.
 */
